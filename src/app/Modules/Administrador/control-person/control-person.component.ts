...
export class ControlPersonComponent implements OnInit {
  ...
  public validateEmailAndCi(identificacion: string, email: string) {
    // funcion para validación de cédula
    function validarCedula(cedula: string): boolean {
      ...
      if (cedula !== '' && longitud === 10) {
        for (let i = 0; i < longcheck; i++) {
          if (i % 2 === 0) {
            var aux = parseInt(cedula.charAt(i)) * 2;
            if (aux > 9) aux -= 9;
            total += aux;
          } else {
        ...
        return cedula.charAt(longitud - 1) === total.toString();
      }
      return false;
    }

    // Validación de cédula
    const cedulaValida = validarCedula(identificacion);

    // Validación de correo electrónico
    const identificacionEqualsValidate =
      this.validateEquals.identificacion === this.person.identificacion;
    const emailEqualsValidate =
      this.validateEquals.emailValidate === this.person.correo;

    const existIdentificacion = this.personService.existByIdentificacion(
      identificacion,
      identificacionEqualsValidate
    );
    const existEmail = this.personService.existsByEmail(
      email,
      emailEqualsValidate
    );
    ...
    forkJoin([existIdentificacion, existEmail]).subscribe(
      ([cedulaRepetidaResp, emailRepetidoResp]) => {
        this.respose.ci = cedulaValida ? '' : 'Cédula Inválida';
        this.respose.ci = cedulaRepetidaResp
          ? 'Identificación existente'
          : this.respose.ci;
        this.respose.emailValidate = emailRepetidoResp
          ? 'Dirección de correo existente'
          : '';

        if (cedulaValida && !cedulaRepetidaResp && !emailRepetidoResp) {
  ...
  public editPerson(person: Persona) {
    ...
    this.person = { ...person };
    this.validateEquals = {
      identificacion: this.person.identificacion,
      emailValidate: this.person.correo,
    };
